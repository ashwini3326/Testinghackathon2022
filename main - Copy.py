...
@app.route('/staticRefreshDataSource', methods=['GET', 'POST'])
def staticRefreshDataSource():
    
    form = LoginForm()
        
    dagParsedOutputTable = "dmgcp-del-108.db_sse_lineage.dag_control_m_data"
    
    try:
        return_code, msg = lineage.download_dag_parsed_output(dagParsedOutputTable)
        return render_template('generateTableLineage.html', form=form, msg = msg, all_ctm=[],all_dags=[])
    except Exception as e:
        return render_template('failure.html', failure_msg = f"Error while refreshing Lineage Data Source Table : {str(e)}; If you think it's a mistake, please contact admin!")
...
def uploadXML():
    ...
    if request.method == 'POST':
        uploaded_files = request.files.getlist('uploadXML')
